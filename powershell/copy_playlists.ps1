# Script Name:    copy_playlists.ps1
# Author:         DJ Differens
# Created:        02-11-2024
# Version:        1.0
# Description:    Script for copying playlists to folder by confirming each playlist individually.
...
if (Test-Path $xmlFilePath) {
	...
	# Loop through all playlists and ask user input. If y then create directory and copy files and when n then skip the playlist
	foreach ($playlist in $playlists.Keys) {
		do {
			$answer = Read-Host "Do you want to copy playlist $playlist ? (y/n)"
			...
			switch ($answer.ToLower()) {
				...
				"n" {
					Write-Host "Skip copying $playlist..."
					# Add the actions to be performed for 'No' answer here
					$valid = $true
				}
				default {
